...
|---|---|---|---|---|
| 1 | 2026-05-06 | Row 31 hardcoded 3-item inline array | Deleted inline array literal; wired to real `documents[]` sorted by `uploadedAt DESC`, top 3. Status labels derived from `category` + age (`Active` for Rental, `Expiring` for Insurance > 1 yr). Empty state renders `"—"`. Phase 6.3. | pending |
| 2 | 2026-05-06 | Rows 5, 8–10, 18–26 — Lease+Tenant entity wiring (Phase 6.1) | Created `getRentalPageData` in `rental/queries.ts`; wired `pageSubtitle`, `isOccupied`/`occupancyValue`/`occupancyAccent`, `rentValue`, `termLabel`, `tenantNameInSummary`, `leaseFields`, `expiryText`, `avatarInitials`, `tenantName`, `tenantEmail`, `tenantPhone`, `movedInDate` inside `PropertyRentalPage`; schema gaps for Deposit and Auto-pay documented as F2/F3 in lease-summary-fields audit | valgate-local-db branch |
| 3 | 2026-05-06 | Rows 11–17, 27, 32, 33 — Payment+Expense entity wiring (Phase 6.2) | **PF4 sprung first**: deleted module-level `chartData` + `payments` constants before any wiring. Built `Expense` type + db + 7 seeds. Extended `getRentalPageData` to fetch `Payment[]` + `Expense[]`. Wired inside `PropertyRentalPage`: chart window `[Nov 2025, May 2026)`, `rentInWindow`/`chartMonths`/`chartData` (6-bar chart), `periodLabel`, `totalRentInWindow`, `expensesInWindow`/`totalExpensesInWindow`, `netIncomeInWindow`, YTD `ytdNetIncome`, `balanceDue`/`balanceDueStr`/`balanceDueAccent`, `onTimePct`, sorted+paged payment table, `displayEnd`/`totalPaymentPages`. Removed "↑ vs prior period" accents (no prior-period query). Q3.B deferred. Per-datapoint audits: property-id-rental--ytd-net-income, --balance-due, --financial-overview-chart, --period-label, --total-rent, --expenses-subtotal, --net-income-subtotal, --on-time-pct, --payment-history, --pagination | valgate-local-db branch |
| 4 | 2026-05-06 | Rows 28–30 — MaintenanceItem entity wiring (Phase 6.8) | Extended `getRentalPageData` to fetch `MaintenanceItem[]` filtered by `propertyId`; wired inside `PropertyRentalPage`: `openMaintenance` (filter status=Open), `openCount` (for badge), `displayItems` (sorted by severityRank DESC then createdAt DESC, sliced to 2); Maintenance card now renders `{openCount} Open` badge + conditional item list with severity dot color + `item.title` + `"{item.severity} priority"` sub-label; empty state renders "—"; Phase 6.2 Financial Overview + Phase 6.3 Documents card untouched | valgate-local-db branch |
