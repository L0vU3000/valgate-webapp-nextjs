---
slug: property-id-rental--tenant-phone
data_point: "Tenant Profile card — phone contact row"
...
# Audit — Tenant Phone on /property/[id]/rental
_Last revised: 2026-05-06 · Revision 1_

## TL;DR
- ✅ Correct — shows "+855 12 345 678" for PROP-0001, matching TEN-0001.phone
...
> **Plain opener:** The Tenant Profile card shows the primary tenant's phone number with a phone icon. For PROP-0001 with TEN-0001 linked, this shows "+855 12 345 678". If there is no linked tenant it shows "—".

| | |
|---|---|
| Where | `/property/PROP-0001/rental`, Tenant Profile card, phone row (Phone icon + text) |
| Label | _none — icon implies label_ |
| Reads | `primaryTenant?.phone ?? "—"` via `tenantPhone` |
| Canonical home | client (`PropertyRentalPage`) |
| Edge case | no active lease or no linked tenant → `"—"` |

**Formula:** `const tenantPhone = primaryTenant?.phone ?? "—"`

## 2. Entity

| Field | Source | Value (PROP-0001) |
|---|---|---|
| `Tenant.phone` | TEN-0001 | `"+855 12 345 678"` |
...
- Golden-value check ✅: TEN-0001.phone = "+855 12 345 678".
