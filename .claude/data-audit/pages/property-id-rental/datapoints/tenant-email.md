---
slug: property-id-rental--tenant-email
data_point: "Tenant Profile card — email contact row"
...
# Audit — Tenant Email on /property/[id]/rental
_Last revised: 2026-05-06 · Revision 1_

## TL;DR
- ✅ Correct — shows "sok.dara@example.com" for PROP-0001, matching TEN-0001.email
...
> **Plain opener:** The Tenant Profile card shows the primary tenant's email address with a mail icon. For PROP-0001 with TEN-0001 linked, this shows "sok.dara@example.com". If there is no linked tenant it shows "—".

| | |
|---|---|
| Where | `/property/PROP-0001/rental`, Tenant Profile card, email row (Mail icon + text) |
| Label | _none — icon implies label_ |
| Reads | `primaryTenant?.email ?? "—"` via `tenantEmail` |
| Canonical home | client (`PropertyRentalPage`) |
| Edge case | no active lease or no linked tenant → `"—"` |

**Formula:** `const tenantEmail = primaryTenant?.email ?? "—"`

## 2. Entity

| Field | Source | Value (PROP-0001) |
|---|---|---|
| `Tenant.email` | TEN-0001 | `"sok.dara@example.com"` |
...
- Golden-value check ✅: TEN-0001.email = "sok.dara@example.com".
