---
slug: property-id-rental--lease-summary-tenant
data_point: "Lease Summary card — tenant name line (below card heading)"
...
# Audit — Lease Summary Tenant Name on /property/[id]/rental
...
> **Plain opener:** Just below the "Lease Summary" heading, the card shows the primary tenant's name in semibold text. For PROP-0001 with TEN-0001 linked, this reads "Sok Dara". If there is no linked tenant it shows "—".

| | |
|---|---|
| Where | `/property/PROP-0001/rental`, Lease Summary card, first line below heading |
| Label | _none — name stands alone_ |
| Reads | `primaryTenant?.name ?? "—"` via `tenantNameInSummary` |
...
// primaryTenant = activeLease?.tenantId ? tenantMap.get(activeLease.tenantId) ?? null : null
```
