...
| 6 | 2026-05-07 | Q5.T resolved; bell panel fully wired (Phase 8.8) | Added `propertyId: z.string().optional()` to `NotificationSchema`; seeds NOTIF-0001..0004 updated with `propertyId`; `notificationMatchesProperty()` updated to prefer `propertyId`, fall back to `linkTo` regex. Bell: `NotificationsContext` + `NotificationsProvider` created; shell layout fetches notifications server-side; `AppHeader` seeded from context; badge conditional on unread count; clicking a row calls `markAsRead` + navigates `linkTo`; "Manage notifications" button routes to `/settings`; `markRead` server action gains Zod validation. | valgate-local-db branch |
