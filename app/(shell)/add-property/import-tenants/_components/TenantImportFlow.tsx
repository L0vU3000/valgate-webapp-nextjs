...
import { mapTenantsAction, bulkCreateTenantsAction } from "@/app/actions/tenant-import";
...
export function TenantImportFlow() {
  ...
  async function handleFile(file: File) {
    ...
    // 1. Parse EVERY sheet to a raw matrix — the field-first engine needs the
    //    whole workbook, since tenant data is split across sheets.
    let sheets;
    try {
    ...
    // 2. Slice each sheet at its header row into header-keyed rows. Drop sheets
    //    with no usable header or no data.
    ...
    // 3. The AI sources the tenant fields across the whole workbook; we get back
    //    reviewable candidates + the org's property list for the picker.
    const res = await mapTenantsAction(sheetData);
    if (!res.ok) {
      setError(res.error);
      setStage("upload");
      return;
    ...
    if (res.data.candidates.length === 0) {
      setError("Couldn't identify any tenants in that workbook. Make sure it has a tenant list.");
      setStage("upload");
      return;
    }
    if (res.data.candidates.length > MAX_IMPORT_ROWS) {
      setError(`That workbook has ${res.data.candidates.length} tenants. Import supports up to ${MAX_IMPORT_ROWS} at a time.`);
      setStage("upload");
      return;
    }
  ...
  async function handleImport(drafts: TenantDraft[]) {
    const res = await bulkCreateTenantsAction(drafts);
    if (!res.ok) {
    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-auto px-4 sm:px-8 py-6">
        <div className="max-w-[1000px] mx-auto">
          ...
          <div className="mb-8">
            <h1 className="text-[28px] font-bold text-slate-900 leading-tight mb-1">Import tenants from spreadsheet</h1>
            <p className="text-sm text-slate-500">
              Upload a client workbook. We&apos;ll read across all its sheets, match each tenant to a
          ...
          {stage === "upload" && (
            <div>
              ...
              >
                ...
                <div className="text-sm text-slate-500">CSV or Excel (.xlsx) · up to {MAX_IMPORT_ROWS} tenants · max 20 MB</div>
              </div>
              ...
              <div className="mt-4 flex items-start gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
                ...
                <span>
                  Sheets can be laid out any way — tenant details and lease/rent info can live in separate
                  sheets. An AI reads across all of them and links each tenant to a property.
                </span>
              </div>
          ...
          {stage === "mapping" && (
            <div className="flex flex-col items-center justify-center gap-3 py-24 text-slate-500">
              ...
              <p className="text-sm">Reading your workbook and matching tenants to properties…</p>
            </div>
          )}
          ...
          {stage === "done" && done && (
            <div className="rounded-2xl border border-slate-200 bg-white p-6">
              <h2 className="text-lg font-semibold text-slate-900">
                {done.created > 0 ? `Imported ${done.created} ${done.created === 1 ? "tenant" : "tenants"}` : "Nothing was imported"}
              </h2>
