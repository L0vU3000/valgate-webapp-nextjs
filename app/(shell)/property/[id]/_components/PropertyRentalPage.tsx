...
// bundle — it downloads once the chart renders. The skeleton fills the same h-[200px] box.
const RentalBarChart = dynamic(
  () => import("./RentalBarChart").then((m) => m.RentalBarChart),
  { ssr: false, loading: () => <div className="h-[200px] w-full animate-pulse rounded-lg bg-slate-50" /> },
);
import {
...
// One bar group per month: rent collected vs expenses. Shared by FinancialOverviewCard and the
// (lazily-loaded) RentalBarChart child.
  ...
  return (
    <div className="lg:col-span-8 bg-white border border-slate-200 rounded-xl overflow-hidden shadow-[0px_1px_4px_0px_rgba(18,28,40,0.06)]">
      ...
      <div className="px-2 pb-1">
        {hasChartData ? (
          <RentalBarChart chartData={chartData} />
        ) : (
