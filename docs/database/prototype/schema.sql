-- =====================================================================
-- Valgate — PostgreSQL DDL prototype (Neon target)
-- Identity: Clerk Organizations — see docs/database/identity.md
--
-- Conventions
--   organizations.id / users.id — Clerk ids (org_…, user_…) in prototype; UUID in production
--   Tenancy — org_id on all portfolio data (NOT user_id)
--   user_id — only org_memberships, user_profiles, notification_preferences, created_by_* audit
--   Timestamps: BIGINT (Unix ms epoch) on domain tables; identity tables use BIGINT until migrated
--   Anomalies: comments tagged A1 / A2 / A3 on domain columns
-- =====================================================================
...
-- =====================================================================
-- IDENTITY (mirrors Clerk — docs/database/identity.md)
-- =====================================================================

-- ---------------------------------------------------------------------
-- organizations — Clerk Organization (client workspace: Org A, B, C)
-- ---------------------------------------------------------------------
CREATE TABLE organizations (
  id            TEXT PRIMARY KEY,       -- prototype: org_… (same as clerk_org_id)
  clerk_org_id  TEXT NOT NULL UNIQUE,   -- org_…
  slug          TEXT UNIQUE,
  name          TEXT NOT NULL,
  metadata      JSONB,
...
-- ---------------------------------------------------------------------
-- users — Clerk User (owner, staff, Bobby, Alice, …)
-- ---------------------------------------------------------------------
CREATE TABLE users (
  id              TEXT PRIMARY KEY,   -- prototype: user_… (same as clerk_user_id)
...
-- ---------------------------------------------------------------------
-- org_memberships — Clerk organization membership (role per org)
-- ---------------------------------------------------------------------
CREATE TABLE org_memberships (
  id                   TEXT PRIMARY KEY,   -- om_…
  clerk_membership_id  TEXT UNIQUE,        -- when present on webhook payload
  org_id               TEXT NOT NULL REFERENCES organizations(id),
  user_id              TEXT NOT NULL REFERENCES users(id),
  role                 TEXT NOT NULL,      -- org:admin | org:member | org:asset_manager | org:lawyer | …
  status               TEXT NOT NULL,      -- active | invited | suspended | removed
  permissions          TEXT[],             -- optional mirror of Clerk custom permissions
...
CREATE INDEX idx_org_memberships_org_status ON org_memberships(org_id, status);

-- =====================================================================
-- DOMAIN (org-scoped portfolio data)
-- =====================================================================
...
CREATE UNIQUE INDEX idx_properties_org_code ON properties(org_id, code);
CREATE INDEX idx_properties_org_id        ON properties(org_id);
CREATE INDEX idx_properties_org_id_status ON properties(org_id, status);
...
CREATE INDEX idx_leases_org_id_stage  ON leases(org_id, stage);
...
CREATE INDEX idx_expenses_org_id_category ON expenses(org_id, category);
...
CREATE INDEX idx_documents_org_id_category ON documents(org_id, category);
...
CREATE INDEX idx_maintenance_items_org_id        ON maintenance_items(org_id);
CREATE INDEX idx_maintenance_items_property_id    ON maintenance_items(property_id);
CREATE INDEX idx_maintenance_items_org_id_status ON maintenance_items(org_id, status);
...
CREATE INDEX idx_successors_org_id ON successors(org_id);
...
CREATE INDEX idx_spa_org_id       ON successor_property_assignments(org_id);
...
CREATE INDEX idx_eae_org_id              ON estate_activity_events(org_id);
CREATE INDEX idx_eae_property_id          ON estate_activity_events(property_id);
CREATE INDEX idx_eae_org_id_created_at   ON estate_activity_events(org_id, created_at);
...
CREATE INDEX idx_professionals_org_id_category   ON professionals(org_id, category);
CREATE INDEX idx_professionals_org_id_verified   ON professionals(org_id, verified);

-- ---------------------------------------------------------------------
-- user_profiles (1→1 with users — not org tenancy)
-- ---------------------------------------------------------------------
...
CREATE INDEX idx_notifications_org_id_read   ON notifications(org_id, read);

-- ---------------------------------------------------------------------
-- notification_preferences (per user — not org tenancy)
-- ---------------------------------------------------------------------
...
-- =====================================================================
-- End of schema. 3 identity tables + 25 domain tables = 28 total.
-- See docs/database/identity.md for Clerk sync and webhooks.
-- =====================================================================
