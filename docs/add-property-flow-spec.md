...
- `Save as Draft` button explicitly writes to Convex `propertyDrafts` table
...
### Submit behavior
- Footer `Submit Property` → loading state → Convex mutation `createProperty` → navigate to Step 6 with new `propertyId`.
...
### Convex
- `propertyDrafts` table: `userId`, `data` (partial property), `updatedAt`, `step`
- `properties` table: created on Step 5 submit
- Files: uploaded to Convex storage or R2, IDs stored on draft/property

### Server actions
- `saveDraft(stepData)` — upsert to `propertyDrafts`
- `createProperty(fullData)` — validates with `fullPropertySchema`, writes to `properties`, returns `propertyId`
...
- `Save as Draft` is available from Steps 1–5; writes current form + step to Convex, stays on page.
...
3. Where do files live — Convex storage or R2? Sizes above suggest R2.
