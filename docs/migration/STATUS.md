...
| # | Service | Why | State |
|---|---|---|---|
| 1 | **Clerk** (auth) | required to leave demo; flip `DEMO_MODE=false` | 🟡 code complete (Phases 1–5), **manual E2E pending (Phase 6)**. Keys in `.env.local`, `DEMO_MODE=false`. Built on Clerk **Future/signals API** (`signIn.password/finalize`, `signUp.verifications.sendEmailCode/verifyEmailCode/finalize`) — NOT classic `create/attemptFirstFactor`. ClerkProvider + middleware `auth.protect()` + login/register(OTP+auto-org)/forgot(reset) + sidebar user+sign-out. tsc/eslint/build green. ▶ user must run signup→OTP→org→portfolio test (agent can't receive emails). Webhook secret set; dashboard webhook endpoint still to add at deploy. Plan: `docs/migration/CLERK-PLAN.md`. |
