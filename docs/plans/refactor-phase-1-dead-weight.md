...
- archive/ (92 files), imports/ (51 files), lib/mock-data.ts deleted — all three verified zero live imports first.
- imports/ decision: deleted (recoverable via git history).
- tsconfig.tsbuildinfo already gitignored (line 50).
- TODO triage: only 1 marker in live code — components/auth/AuthFooter.tsx:1 (confirm legal page routes). No backend TODOs; nothing for Phase 2/4 to carry.
- Note: verification ran alongside uncommitted edit-client WIP from a parallel agent (actions.ts, ClientPageHeader, ClientsTable, client-onboarding.ts, EditClientDrawer.tsx) — excluded from the Phase 1 commit.
