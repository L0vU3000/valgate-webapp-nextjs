...
## Cross-Machine Sync SOP (Hermes ↔ Conductor)

- `.claude/settings.json` registers the Hindsight MCP server at `http://100.77.9.23:8081/sse`.
- `sessionstart.py` runs at session start and injects relevant Hindsight context.
- `userpromptsubmit.py` injects delta context, throttled to once every 30 seconds.
- End every session by committing code, writing a `conductor-logs/YYYY-MM-DD-<task>.md` log with `workspace_state` frontmatter, and pushing the feature branch.
