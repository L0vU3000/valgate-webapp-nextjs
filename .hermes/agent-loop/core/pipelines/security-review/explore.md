...
1. Read the ticket and apply `pipeline.md`'s scope gate. Accept only a request to **review** an
   ...
   defensive review of the app (see STACK.md) only).
...
4. Map the change against the attack surface so the review can be grounded. Use
   ...
   (`FormData`, params, body) enters and whether it is input-validated before a data-layer query; what
   it returns to the client on error; any use of a client-bundled/public env var (e.g. a `NEXT_PUBLIC_`-style prefix) or secrets/props crossing into a
   Client Component; and any sensitive action (login, signup, invite, payment) that would need a
   rate limit. Note the standing project conventions doc **Security Rules** each touchpoint must satisfy
   (authN + authZ + ownership on every mutation, validate all input, never leak `err.message`,
   no secrets in a public env prefix, no full DB objects as props, rate-limit sensitive actions). Record
...
architecture or design audit, a target with no diff to inspect, or an offensive/non-project
