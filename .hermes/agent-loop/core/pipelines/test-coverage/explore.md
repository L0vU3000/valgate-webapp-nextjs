...
1. **Map what's untested** — list the services-layer (see STACK.md) modules and which already have a
...
3. **Pick the test lane** (see `pipeline.md`): default DB-free lane (mock `db` / pure logic)
   or `*.db.test.ts` live-DB lane (the **dev** database only — confirm the dev database
   connection string (e.g. `DATABASE_URL`) in the dev environment config is not the prod endpoint
   before choosing this lane).
