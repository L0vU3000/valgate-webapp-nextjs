...
2. The **finding shape** every finding must follow: `severity · location (file:line or module) ·
   cited evidence · why it matters`. Cited evidence names the exact rule in the project conventions doc (see STACK.md) the code
...
3. The **severity definitions** for this structure, so severity is graded against a fixed bar and
   ...
   through the services layer, business logic in a route handler, a client component importing a
   server secret, a live import into a dead/parallel backend layer, or a cyclic dependency between modules;
...
5. The **grounding list**: every real file/module and the specific rule in the project conventions doc or dependency
