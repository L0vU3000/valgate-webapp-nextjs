...
2. **Confirm it is not already built.** Run `graphify query` for the capability and its closest
   siblings, then grep the services layer (see STACK.md), the server-actions directory, the schema directory, and
   the derivations directory for the concept — not just for a same-named table or file. A missing
...
4. Gather the real codebase context the spec must be grounded in: the existing pattern for this
   ...
   services and actions involved, and the standing constraints from the project conventions doc that apply.
