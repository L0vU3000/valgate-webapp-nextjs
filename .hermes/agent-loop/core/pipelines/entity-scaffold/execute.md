...
1. Implement the validation-layer contract (see STACK.md) exactly as approved. Export the domain, new-input, and patch types.
2. Add the schema-definition table with non-null `orgId`, `userId`, and `propertyId`; property cascade;
...
3. Add the entity service. All reads filter `orgId`; writes use `_crud.ts`; create and any
   ...
   No component or route handler queries the data layer directly.
...
5. Add the seed loader entry, complete table list entry, live schema assertion count/inventory,
   and one meaningful fixture. Never run a destructive seed reset.
...
2. Confirm the database endpoint is the approved dev database and recompute the
...
shape, the generated migration includes unrelated drift, the dev database cannot be
