...
2. Resolve the caller through **the request-context seam** (the read/write context resolvers, which
   require an explicit org for a write). Validate input with the **existing** validation-layer schema (see STACK.md)
...
- If the wrapped service touches data, use **the dev database** — never prod, never run
  a destructive seed reset.
