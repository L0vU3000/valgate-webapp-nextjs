...
> a function in the services layer (see STACK.md) through the established request-context seam. It writes the tool
...
A new tool exists on the external surface that calls a real function in the services layer through
the request-context seam, validates its input with the validation layer, inherits the service's authorization, hides internal
...
2. The tool resolves its caller through **the request-context seam** and calls an **existing** service
   function — it re-implements no business logic and adds no schema.
3. **Input is validated by the validation layer** before the service is touched; malformed input is refused.
...
the way a caller reaches it — through the request-context seam into the real service — so "works" means the
...
the validation layer before any DB work. This matches what the pipeline produces — an external
...
- **explore = ground + specify.** Confirm the target function in the services layer already exists
  and read how its siblings are wrapped in the tool-registration layer (see STACK.md).
...
- **plan** = the smallest wiring that satisfies the tool test, reusing the site's existing
  validation-layer schema and the shared read/write context resolvers. Plan also writes the
  100-point rubric.
...
- **Data safety:** if the wrapped service touches data, exercise it against **the dev
  database** — never prod, **never run a destructive seed reset**.
...
- **Reuse authorization, never re-implement it:** the tool must let the service's own
  org-scope, role, and demo read-only guards run through the request-context seam. Writes resolve their Ctx
...
- **Input validated** — the validation layer refuses malformed input before the DB is touched.
- **No error leakage** — no raw `err.message` reaches the caller; details are logged internally.
- **Tool works end-to-end** through the request-context seam against the real service (red→green).
