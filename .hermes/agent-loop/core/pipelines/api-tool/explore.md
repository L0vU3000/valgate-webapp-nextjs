...
already exists in the services layer (see STACK.md), learn how sibling tools wrap it, and turn the ticket into a
**tool test that fails now** — not to build the tool.

## Your job

1. **Confirm the service exists.** The tool wraps an existing function in the services layer. Use
...
2. **Learn the wrapping pattern.** Read how peers wrap services on the same surface — the
   request-context seam and the tool-registration layer (see STACK.md): how the read/write context
   resolvers resolve the caller, which validation-layer schema the site already uses for this input,
   how results and generic errors are shaped, and where writes require an explicit org.
3. **Write the tool test** — a focused end-to-end test that drives the tool the way a caller
   reaches it (through the request-context seam into the real service), covering three probes:
   ...
   - **malformed input** — bad input is **refused by the validation layer** before any DB work.
   Run the test and confirm it is **red for the right reason** (the tool is absent), not an
   import or setup error.
4. Write to `runs/<run-id>/explore.md`: the target service function, the site validation-layer schema to
