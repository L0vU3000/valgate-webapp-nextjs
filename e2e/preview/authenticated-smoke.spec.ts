...
import { findForbiddenWords } from './lib/preview-checks'
...
test.describe('Preview authenticated smoke', () => {
  ...
  test('direct "/" resolves to /app for a signed-in session', async ({ page }) => {
    await page.goto('/')
    await expect(page).toHaveURL(/\/app(?:[/?#]|$)/)
  })

  test('/settings has no Claude/MCP/AI/Connect copy', async ({ page }) => {
  ...
  test('/add-property landing screen has no marketplace/rental copy', async ({ page }) => {
  ...
  test('Get Started shows method choice directly (no interstitial dialog)', async ({ page }) => {
  ...
  test('Enter manually -> Continue reaches a property-type screen with Residential House', async ({ page }) => {
    await page.goto('/add-property')
    await page.getByRole('button', { name: /get started/i }).first().click()
