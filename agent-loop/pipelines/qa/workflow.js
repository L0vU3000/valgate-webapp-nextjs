...
//
// Optionally pass routes as args, e.g. Workflow({scriptPath, args: '/home, /documents'})
...
const VERDICT = { type: 'object', required: ['verdict', 'flowsPassing', 'consoleClean', 'suiteGreen', 'tscErrors'],
  properties: {
...
const drive = await agent(
  `You are the EXPLORE stage of the qa pipeline. Follow ${P}/explore.md.
   ...
   Use ToolSearch to load the Playwright browser tools. Reuse the dev server on port 3002 if
   it answers; otherwise start \`npm run dev:e2e\` in the background. NEVER wait on
...
if (drive.findingCount === 0) {
  log(`run ${RUN} — all in-scope flows clean, nothing to fix.`)
...
while (i < MAX) {
  ...
  const v = await agent(
    `You are the EVAL stage (VERIFIER — a DIFFERENT agent from the maker). Follow ${P}/eval.md.
     ...
     Re-drive every in-scope flow from \`${P}/runs/${RUN}/explore.md\`; console must be free of
     errors; then \`npx vitest run\` (green), \`npx tsc --noEmit\` (0), \`${LINT}\` (no new
     warnings). Run the anti-gaming diff review from eval.md. Return the verdict with cited
     evidence.`,
    { label: `eval#${i}`, phase: 'Fix loop', schema: VERDICT, model: 'sonnet' })

  log(`iter ${i}: flows ${v.flowsPassing ? 'pass' : 'FAIL'} · console ${v.consoleClean ? 'clean' : 'DIRTY'} · suite ${v.suiteGreen ? 'green' : 'RED'} · tsc ${v.tscErrors}`)

  if (v.verdict === 'pass' && v.flowsPassing && v.consoleClean && v.suiteGreen && v.tscErrors === 0) {
